...
Wall_Cache()
{
    ...
    # Keep wallpaper switching independent from dynamic theme recoloring.
    # This prevents terminal/UI colors from changing on every wallpaper update.
...
if command -v swww &> /dev/null && command -v swww-daemon &> /dev/null ; then
    wallCmd="swww"
    wallDaemonCmd="swww-daemon"
elif command -v awww &> /dev/null && command -v awww-daemon &> /dev/null ; then
    wallCmd="awww"
    wallDaemonCmd="awww-daemon"
else
    echo "ERROR: neither swww nor awww is installed or available in PATH"
    exit 1
fi
...
"${wallCmd}" query &> /dev/null
if [ $? -ne 0 ] ; then
    "${wallDaemonCmd}" --format xrgb &
    daemon_ready=0
    for _ in $(seq 1 50) ; do
        if "${wallCmd}" query &> /dev/null ; then
            daemon_ready=1
            break
        fi
        sleep 0.1
    done
    [ "${daemon_ready}" -eq 1 ] && "${wallCmd}" restore &> /dev/null
fi
...
[ -n "${wallTransType}" ] && xtrans="${wallTransType}"
if [ "${wallTransType}" = "random" ] ; then
    if command -v shuf &> /dev/null; then
        xtrans="$(printf '%s\n' fade grow outer wipe any | shuf -n 1)"
    else
        xtrans="$(printf '%s\n' fade grow outer wipe any | awk 'BEGIN{srand()} {a[NR]=$0} END{print a[int(rand()*NR)+1]}')"
    fi
fi
...
if ! "${wallCmd}" query &> /dev/null ; then
    echo "ERROR: wallpaper daemon did not become ready"
    exit 1
fi
"${wallCmd}" img "$(readlink "${wallSet}")" --resize crop --transition-bezier .43,1.19,1,.4 --transition-type "${xtrans}" --transition-duration "${wallTransDuration}" --transition-fps "${wallFramerate}" --invert-y --transition-pos "$(hyprctl cursorpos | grep -E '^[0-9]' || echo "0,0")" &
